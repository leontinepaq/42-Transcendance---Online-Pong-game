...
for i in range(0,100):
    ...
    if random.randint(0, 1):
        ...
        score_2 = 3
    if random.randint(0, 1):
        game = Game.create_ai(ids[random.randint(0, 3)], score_1, score_2)
    else:
        players = random.sample(ids, 2)
        game = Game.create(players[0], players[1], score_1, score_2)
    game.created_at = datetime.datetime.now() - datetime.timedelta(days = random.randint(0, 10))
    game.longest_exchange = random.randint(0, 100)
    game.duration = datetime.timedelta(minutes=random.randint(0, 5),
