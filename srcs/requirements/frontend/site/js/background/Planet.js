export class Planet {
	...
	update(canvas) {
		if (this.exiting)
			this.exitUpdate(canvas);
		else if (this.entering)
			this.entryUpdate();
		else
			this.normalUpdate(canvas);		
	}
	...
	entryUpdate() {
		...
		if (this.entrySide === 'left')
			this.x = -0.2 + (finalX + 0.2) * t;
		else 
			this.x = 1.2 - (1.2 - finalX) * t;
		this.y += this.descentSpeed;
	}
	...
	exitUpdate(canvas) {
		...
		if (this.y > 1 + this.radius / canvas.height || this.x < -this.radius / canvas.width || this.x > 1 + this.radius / canvas.width)
			this.exited = true;
	}
	...
	draw(ctx, canvas) {
		...
		ctx.beginPath();
		ctx.arc(this.x * canvas.width, this.y * canvas.height, this.radius, 0, Math.PI * 2);
		ctx.fill();
