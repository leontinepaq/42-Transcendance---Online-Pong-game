...
export const SkyAnimation = {
	...
	launch() {
		...
		window.addEventListener("resize", () => {setupCanvas(this.canvas);});
	},
	...
	tryCreateShootingStar(ts) {
		if (ts - this.lastShootingStarTs < 2000)
			return;
		if (Math.random() > 0.7)
	...
	animate(ts = performance.now()) {
		...
		this.shootingStars = this.shootingStars.filter(shootingStar => shootingStar.opacity > 0);
		this.stars.forEach(star => star.update(deltaTime));
		this.shootingStars.forEach(shootingStar => shootingStar.update(deltaTime));
		
		this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
		
		this.stars.forEach(star => star.draw(this.ctx, this.canvas));
		this.shootingStars.forEach(shootingStar => shootingStar.draw(this.ctx, this.canvas));
